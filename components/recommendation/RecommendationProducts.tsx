    ...
    <section className="min-h-screen flex flex-col items-center py-24 px-4 bg-gradient-to-b from-white via-orange-50/30 to-white">
      <div className="max-w-7xl w-full mx-auto p-8">
        <div className="text-center mb-20 animate-fade-in">
          <div className="uppercase text-sm text-orange-400 font-semibold mb-4 tracking-[0.2em] animate-slide-down">Curated Selection</div>
          ...
          <p className="text-gray-600 text-xl max-w-2xl mx-auto leading-relaxed animate-fade-in">
            Discover our meticulously curated collection of premium wellness products, crafted for those who seek excellence in every detail.
          </p>
        </div>
                  ...
                  <div className="flex justify-center">
                    {product.url ? (
                      ...
                      <div className="flex flex-col items-center gap-4">
                        <div className="text-sm text-gray-500">Use this coupon code:</div>
                        <Button
                          onClick={() => product.coupon_code && handleCopyCode(product.coupon_code)}
                        ...
                        >
                          {product.coupon_code}
                          <svg 
                            className={`w-5 h-5 transition-transform duration-300 ${copiedCode === product.coupon_code ? 'text-green-500' : ''}`}
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                          >
                        ...
                        {copiedCode === product.coupon_code && (
                          <Button
                            onClick={() => window.open(product.redirect_url, "_blank")}
                            className="text-base font-semibold text-orange-400 border-2 border-orange-400 rounded-full hover:bg-orange-400 hover:text-white transition-all duration-300 transform hover:scale-105 hover:shadow-lg bg-white cursor-pointer"
                          >
                            Visit Website
                          </Button>
